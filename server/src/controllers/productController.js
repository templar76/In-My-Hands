...
export const getProductsStats = async (req, res) => {
  try {
    ...
    // Sostituisci completamente la pipeline statsAggregate (linee 325-353) con:
    ...
    // Rimuovi anche le linee 355-357 che aggiungono il filtro data duplicato:
    // if (Object.keys(dateFilter).length > 0) {
    //   monthlyTrendAggregate.push({ $match: { invoiceDate: dateFilter } });
    // }

    // Pipeline per i trend mensili - AGGIUNGI QUESTA DEFINIZIONE
...
export const getProductDetails = async (req, res) => {
  try {
    ...
    // Sostituisci le aggregazioni esistenti (linee ~492-520, ~525-570, ~572-610)
    
    // Aggregazione per Overview - volumi mensili (CORRETTA)
    ...
    // Aggregazione per Purchase History (CORRETTA)
    ...
    unitOfMeasure: '$lineItems.unitOfMeasure'  // Corretto
    }
    },
    {
    $sort: { invoiceDate: -1 }
    ...
    // Aggregazione per Savings (CORRETTA)
    ...
    console.log('Overview data length:', overviewData.length);
    console.log('Overview data:', JSON.stringify(overviewData, null, 2));
    console.log('Purchase history length:', purchaseHistory.length);
    console.log('Savings data length:', savingsData.length);
    console.log('============================');
...
// Nella funzione di aggiornamento prezzi
...
// Dopo l'aggiornamento del prezzo, controlla gli alert
await alertMonitoringService.checkProductAlerts(productId, req.user.tenantId);
