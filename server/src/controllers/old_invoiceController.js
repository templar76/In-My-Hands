...
import { processFile, processFiles } from '../services/parsers/dispatcher_import.js';
...
// Funzione helper per salvare una fattura da dati parsati
const saveInvoiceFromParsedData = async (invoiceData, tenantId) => {
  ...
  const pivaMatches = invoiceData.cliente.pIva && invoiceData.cliente.pIva === tenant.vatNumber;
  const cfMatches = invoiceData.cliente.codiceFiscale && invoiceData.cliente.codiceFiscale === tenant.codiceFiscale;
  
  if (!pivaMatches && !cfMatches) {
    throw new Error('Dati cliente P.IVA o C.F. non corrispondono ai dati del tenant');
  }
  
  // Trova o crea il supplier
  const supplier = await findOrCreateSupplier({
    name: invoiceData.fornitore.name,
    pIva: invoiceData.fornitore.pIva,
    codiceFiscale: invoiceData.fornitore.codiceFiscale,
    pec: invoiceData.fornitore.pec,
    sdiCode: invoiceData.fornitore.sdiCode,
    address: invoiceData.fornitore.address,
    iscrizioneREA: invoiceData.fornitore.iscrizioneREA,
    contatti: invoiceData.fornitore.contatti
  }, tenantId);
  
  // Controllo anti-duplicazione fattura
  const invoiceDate = new Date(invoiceData.header.data || new Date());
  const existingInvoice = await InvoiceModel.findOne({
    supplierId: supplier._id,
    invoiceNumber: invoiceData.header.numero || 'N/A',
    invoiceDate: invoiceDate
  });
  
  if (existingInvoice) {
    throw new Error(`Fattura già presente: ${invoiceData.header.numero}`);
  }
  ...
  const savedInvoice = await InvoiceModel.create({
    tenantId,
    supplierId: supplier._id,
    transmissionData: invoiceData.transmissionData,
    supplier: {
      name: invoiceData.fornitore.name,
      pIva: invoiceData.fornitore.pIva,
      codiceFiscale: invoiceData.fornitore.codiceFiscale,
    ...
    customer: {
      ...
      pIva: invoiceData.cliente.pIva,
      codiceFiscale: invoiceData.cliente.codiceFiscale,
      pec: invoiceData.cliente.pec,
      address: invoiceData.cliente.address
    },
    documentReferences: invoiceData.documentReferences,
    datiDDT: invoiceData.datiDDT,
    datiTrasporto: invoiceData.datiTrasporto,
    allegati: invoiceData.allegati,
    invoiceNumber: invoiceData.header.numero || 'N/A',
    invoiceDate: invoiceDate,
    totalAmount: parseFloat(invoiceData.header.totale || 0),
    totalVAT: parseFloat(invoiceData.header.totaleIVA || 0),
    currency: invoiceData.header.valuta || 'EUR',
    lineItems: invoiceData.lines.map(line => ({
      lineNumber: line.numeroLinea || null,
    ...
    vatSummary: invoiceData.riepilogo.map(r => ({
    ...
    paymentData: invoiceData.pagamenti.map(p => ({
    ...
    path: null, // Path will be set by the dispatcher
    rawMetadata: {
      header: invoiceData.header,
      lines: invoiceData.lines,
      riepilogo: invoiceData.riepilogo,
      pagamenti: invoiceData.pagamenti,
      transmissionData: invoiceData.transmissionData,
      documentReferences: invoiceData.documentReferences,
      datiDDT: invoiceData.datiDDT,
      datiTrasporto: invoiceData.datiTrasporto,
  ...
  // Importa automaticamente i prodotti
  await importProductsFromInvoice(
    {...invoiceData, invoiceId: savedInvoice._id}, 
    supplier._id, 
    tenantId, 
    savedInvoice._id, 
    tenantConfig
...
export const uploadInvoices = async (req, res) => {
  try {
    const files = req.files || [req.file]; // Supporta sia single che multi-file
    ...
    if (!tenantId) {
      return res.status(400).json({ error: 'Tenant ID non trovato' });
    }
    
    // Processa tutti i file con il dispatcher
    const results = await processFiles(files.map(f => ({
      filename: f.originalname,
      buffer: f.buffer,
      mimetype: f.mimetype
    })));
    
    // Salva le fatture processate con successo
    const savedInvoices = [];
    const errors = [];
    
    for (const result of results) {
      if (result.successo && result.risultato) {
        try {
          // Usa la logica esistente per salvare la fattura
          const savedInvoice = await saveInvoiceFromParsedData(result.risultato, tenantId);
          savedInvoices.push({
    ...
    logger.error('Errore upload fatture', { error: error.message });
...
export const uploadInvoicesWithTracking = async (req, res) => {
  try {
    ...
    const job = await ProcessingJobService.createJob(
      ...
      files,
      {
        uploadMethod: files.length > 1 ? 'multiple' : 'single',
    ...
    processFilesAsync(job.jobId, files, tenantId)
      .catch(error => {
        logger.error('Errore elaborazione asincrona', { 
          jobId: job.jobId, 
          error: error.message 
        });
      });
    ...
    res.json({
      ...
      trackingUrl: `/api/invoices/processing/${job.jobId}`
    });
...
// Funzione asincrona per l'elaborazione
const processFilesAsync = async (jobId, files, tenantId) => {
  await ProcessingJobService.updateJobCounters(jobId);
  
  for (const file of files) {
    const filename = file.originalname;
    
    try {
      // Aggiorna stato: iniziato
      await ProcessingJobService.updateFileProgress(jobId, filename, {
        status: 'processing',
        stage: 'validation',
        percentage: 10,
      ...
      // Aggiorna stato: estrazione
      await ProcessingJobService.updateFileProgress(jobId, filename, {
        stage: 'extraction',
        percentage: 30,
        message: 'Estrazione contenuto'
      });
      
      // Processa file con dispatcher
      ...
      await ProcessingJobService.updateFileProgress(jobId, filename, {
        stage: 'parsing',
        percentage: 60,
        message: 'Parsing dati fattura'
      });
      
      if (result.successo && result.risultato) {
        // Aggiorna stato: salvataggio
        await ProcessingJobService.updateFileProgress(jobId, filename, {
          stage: 'saving',
          percentage: 80,
          message: 'Salvataggio fattura'
        });
        
        // Salva fattura
        const savedInvoice = await saveInvoiceFromParsedData(
          result.risultato, 
          tenantId
        );
        ...
        await ProcessingJobService.updateFileProgress(jobId, filename, {
          ...
          error: {
            code: 'PARSING_ERROR',
            message: result.errore || 'Errore sconosciuto nel parsing',
            details: result
          }
        });
      }
...
// Endpoint per ottenere lo stato di elaborazione
export const getProcessingStatus = async (req, res) => {
  try {
    ...
    if (job.tenantId.toString() !== tenantId) {
    ...
    logger.error('Errore recupero stato elaborazione', { error: error.message });
    res.status(500).json({ error: error.message });
  }
};

// Endpoint per ottenere tutti i job di elaborazione
export const getProcessingJobs = async (req, res) => {
  try {
    ...
    logger.error('Errore recupero job elaborazione', { error: error.message });
    res.status(500).json({ error: error.message });
  }
};